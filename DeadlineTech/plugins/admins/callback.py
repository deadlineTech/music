...
from DeadlineTech.utils.inline import close_markup, stream_markup, stream_markup_timer
...
async def del_back_playlist(client, CallbackQuery, _):
        ...
        if not is_non_admin:
            if CallbackQuery.from_user.id not in SUDOERS:
                ...
                if not admins or CallbackQuery.from_user.id not in admins:
                    return await CallbackQuery.answer(_["admin_14"] if admins else _["admin_13"], show_alert=True)
    ...
    if command == "Pause":
        ...
        if "live_" in queued:
            ...
            try:
                await Anony.skip_stream(chat_id, link, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
                
            button = stream_markup(_, chat_id)
            run = await CallbackQuery.message.reply_text(
                text=_["stream_1"].format(f"https://t.me/{app.username}?start=info_{videoid}", title[:23], duration, user),
            ...
            try:
                await Anony.skip_stream(chat_id, file_path, video=status)
            except:
                return await mystic.edit_text(_["call_6"])
            ...
            try:
                await Anony.skip_stream(chat_id, queued, video=status)
            except:
                return await CallbackQuery.message.reply_text(_["call_6"])
            ...
            if videoid in ["telegram", "soundcloud"]:
                run = await CallbackQuery.message.reply_text(
                    text=_["stream_1"].format(config.SUPPORT_CHAT, title[:23], duration, user),
                    reply_markup=InlineKeyboardMarkup(button),
                )
            else:
                run = await CallbackQuery.message.reply_text(
